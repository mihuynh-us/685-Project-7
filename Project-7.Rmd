...
-   Here, our correlation is a large negative number. This suggests no relationship.
-   The goal of correlation & predicted model is to show that there is no relation between the predicted y hat (x) and the residuals of the model (y). Regressing residual of the model on fitted model, there is no relationship with a p-value of 1.
-   var.test is testing the null hypothesis that the variances of the two groups are equal. The p-value is significant (0.03), so we reject the null hypothesis and conclude that the mean variances are not equal, at the arbitrary cut off of 20 for the fitted model values.
-   So we conclude there there is no constant variance on #1 and #4. The no relationship observed in #2 and #3 is attributed the smaller sample size of 47.
...
Partial residual plot = (yj - effect of other predictors) against xj.
